...
import { SectionTitle, SectionWrapper } from "@/components/shared";
    ...
    <>
      <SectionWrapper className="relative overflow-hidden bg-gradient-to-br from-brand-50 via-white to-slate-100 pt-20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800">
        ...
        <div className="relative grid gap-8 md:grid-cols-[1.1fr_0.9fr] md:items-center">
          ...
          <aside className="rounded-2xl border border-slate-300 bg-white p-6 shadow-card dark:border-slate-700 dark:bg-slate-900">
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Navegacion por anclas</h2>
      ...
      <SectionWrapper>
        <SectionTitle
          ...
          title={`Opciones de ${area.label.toLowerCase()}`}
          description="Cada bloque tiene su ancla para compartir links directos con clientes."
        />
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {area.details.map((detail) => (
            <article
              key={detail.id}
              id={detail.id}
              className={`service-anchor ${areaAnchorClasses[area.id]} scroll-mt-28 rounded-2xl border border-slate-300 bg-white p-5 shadow-card dark:border-slate-700 dark:bg-slate-900`}
            >
